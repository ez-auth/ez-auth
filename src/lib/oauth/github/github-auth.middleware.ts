...
import { GithubAuthFlow } from "./github-auth-flow";
import type { GitHubScope } from "./github-auth.type";

export const githubAuthMiddleware = (options: {
  client_id: string;
  ...
  return async (c, next) => {
    const newState = generateState(c);

    // Create new Auth instance
    const auth = new GithubAuthFlow({
